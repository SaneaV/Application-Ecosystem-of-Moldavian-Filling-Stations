...
public class TelegramGlobalExceptionCoverageTest {
    ...
    void shouldWrapBindExceptions() {
        final TelegramErrorDescription expected = new TelegramErrorDescription(null, BIND_EXCEPTION_MESSAGE,
                getMainMenuKeyboard());

        final BindException bindException = new BindException(Update.class, "update");
        final ResponseEntity<ErrorDescriptionResponse> response = telegramExceptionWrappingStrategy
                .handleBindException(bindException, mock(WebRequest.class));
