...
public class FuelStationPageWrapperTest {
  ...
  private static final FuelStationDto FUEL_STATION_FORTRESS = new FuelStationDto("Fortress", 25.75, 22.3, 13.45,
      46.34746746138542, 28.947447953963454);
  private static final FuelStationDto FUEL_STATION_PETROM = new FuelStationDto("Fortress", 25.75, 22.3, 13.45, 46.34746746138542,
      28.947447953963454);
  ...
  void shouldWrapAllFuelStations(int pageLimit, int offset, int finalSize, List<FuelStationDto> finalList) {
    when(fuelStationFacade.getAllFuelStations(anyDouble(), anyDouble(), anyDouble(), anyInt())).thenReturn(
        List.of(FUEL_STATION_FORTRESS, FUEL_STATION_PETROM));
  ...
  void shouldWrapBestFuelPriceStations(int pageLimit, int offset, int finalSize, List<FuelStationDto> finalList) {
    when(fuelStationFacade.getBestFuelPrice(anyDouble(), anyDouble(), anyDouble(), anyString(), anyInt())).thenReturn(
        List.of(FUEL_STATION_FORTRESS, FUEL_STATION_PETROM));

    final PageDto<FuelStationDto> result = fuelStationPageWrapper.wrapBestFuelPrice(LATITUDE, LONGITUDE, RADIUS, FUEL_TYPE, LIMIT,
        pageLimit, offset);
