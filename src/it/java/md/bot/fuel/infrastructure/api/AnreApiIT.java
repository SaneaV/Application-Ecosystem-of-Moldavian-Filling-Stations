...
public class AnreApiIT {
    ...
    @Test
    @DisplayName("Should return list of fuel stations")
    void shouldReturnListOfFuelStation() {
        final FuelStation fuelStation1 = new FuelStation("ROMPETROL", 25.02, 21.01, 13.49, 48.26629334102463, 26.62967347295055);
        final FuelStation fuelStation2 = new FuelStation("Agro Destinație", 25.02, 21.01, null, 48.27022290028225, 26.68362775316868);

        wireMock.stubFor(get(urlEqualTo(PATH)).willReturn(aResponse()
                .withHeader(CONTENT_TYPE, APPLICATION_JSON_VALUE)
                .withBody(ANRE_RESPONSE)));

        final List<FuelStation> fuelStationsInfo = anreApi.getFuelStationsInfo();

        assertThat(fuelStationsInfo).hasSize(2);
        assertThat(fuelStationsInfo.get(0)).usingRecursiveComparison().isEqualTo(fuelStation1);
        assertThat(fuelStationsInfo.get(1)).usingRecursiveComparison().isEqualTo(fuelStation2);
    }
}
