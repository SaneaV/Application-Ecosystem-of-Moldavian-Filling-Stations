...
public class PreviousFillingStationCallback implements Callback {

  private static final String CALLBACK_QUERY = "PREVIOUS_PAGE";
  ...
  public List<? extends PartialBotApiMethod<?>> execute(CallbackQuery callbackQuery) {
    ...
    final int previousOffset = Integer.parseInt(callbackHolder.getCallbackDataBy(OFFSET)) - 1;
    log.info("Get all filling stations (previous page) in radius for user = {} and offset = {}", userId, previousOffset);
    ...
    if (ALL_FILLING_STATIONS_IN_RADIUS.getCommandId() == command) {
      fillingStation = fillingStationFacade.getAllFillingStations(latitude, longitude, radius, previousOffset);
      hasNext = fillingStationFacade.hasNext(latitude, longitude, radius, previousOffset);
      messageText = toMessage(fillingStation, translatorService, language);
    } else {
      fillingStation = fillingStationFacade.getBestFuelPrice(latitude, longitude, radius, fuelType, previousOffset);
      hasNext = fillingStationFacade.hasNext(latitude, longitude, radius, previousOffset, fuelType);
      messageText = toMessage(fillingStation, fuelType, translatorService, language);
    }

    final InlineKeyboardMarkup replyKeyboard = getInlineKeyboardForAllFillingStations(command, previousOffset, hasNext,
